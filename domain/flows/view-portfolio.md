...
- **Authenticated User**: A logged-in user who wants to sell a stock holding.
- **Guest Browser**: The React frontend displaying the context menu and opening the sell panel.
...
| # | Actor | Action | Description |
|---|-------|--------|-------------|
| 1 | Authenticated User | Right-click a stock row | Right-clicks any stock holding row in the holdings table. |
| 2 | Guest Browser | Show context menu | Displays a context menu with a single option: "Sell". Cash rows do not expose this menu. |
| 3 | Authenticated User | Click "Sell" | Selects the "Sell" option from the context menu. |
| 4 | Guest Browser | Open sell panel | Initiates the sell flow. See `domain/flows/sell-stock` (Flows A, B, C, D). The `ticker` and `quantity` (as `maxQuantity`) are passed from the portfolio row into the sell panel. |

### Postconditions

- The sell panel is open for the selected ticker.
- The sell flow continues in `domain/flows/sell-stock`.
...
- Right-clicking a stock row shows a context menu with a "Sell" option. Right-clicking the cash row does nothing.
