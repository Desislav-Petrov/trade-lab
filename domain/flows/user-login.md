...
Allows a guest to identify themselves and establish a session as a registered user. In this initial implementation, the system presents a list of available email addresses and the guest selects one — no password or credential check is performed. Authentication will be introduced in a future iteration to replace the email selection step.
...
| 7 | System | Return session | Responds to the guest with the session context and the resolved user's `userId`. |
...
> **Temporary implementation.** The email selection step (steps 2–3) is a placeholder. Once authentication is introduced, it will be replaced with a credential submission and verification step. The session establishment (step 5) and all downstream steps are expected to remain unchanged.
