...
|----|---------------|---------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| 1  | Guest Browser | Render registration form  | Displays input fields for `firstName`, `lastName`, `address` (optional), and `email`, and a Submit button.                            |
| 2  | Guest         | Fill in form fields       | Enters values for `firstName`, `lastName`, and `email`. `address` is optional.                                                        |
...
| 5  | System        | Validate required fields  | Checks that `firstName`, `lastName`, and `email` are all present and non-empty. `address` is accepted when supplied but not required.  |
...
| 8  | System        | Create user record        | Generates a new `id` (UUID), sets `status` to `active`, and persists the user with `createdAt` and `updatedAt` set to current timestamp. `address` is stored if supplied; `null` if omitted. |
...
|------------------------|-----------------------------------------------------------------------------|-----------------|------------------------------------------------------------------------------------------------------------------------|
| Missing required field | Any of `firstName`, `lastName`, or `email` is absent or empty               | HTTP 400        | Flow halts at step 5. Inline validation error displayed below each missing field. Submit button re-enabled.            |
...
- **User**: Created at step 8 with all provided fields and a system-generated `id`. `address` is stored if supplied; `null` if omitted.
